...
export function mountIde() {
  ...
  const files: Record<string, { title: string; body: string[] }> = {
    readme: {
      title: "README.MD",
      body: [
        "# Halyard (working copy)",
        "",
        "A studio operating file. Clone it, rename it, ship from it.",
        "Decisions stay committed so the next session does not redraw the map.",
      ],
    },
    agents: {
      title: "AGENTS.MD",
      body: [
        "# Agents",
        "",
        "- Do not invent a new folder tree.",
        "- Prefer existing section primitives.",
        "- Keep fetch + revalidate in /lib/content.",
      ],
    },
    schema: {
      title: "SCHEMA.TS",
